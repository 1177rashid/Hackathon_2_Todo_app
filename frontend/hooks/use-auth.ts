  ...
  const login = useCallback(async (email: string, password: string) => {
    try {
      ...
      if (response.data) {
        ...
        // Get JWT token from our server-side API endpoint
        // This endpoint will generate a properly signed JWT using Better Auth session
        let token: string
        ...
        try {
          ...
          if (tokenResponse.ok) {
            ...
            console.log('✅ Got JWT token from server:', token.substring(0, 50) + '...')
          } else {
            throw new Error(`Token API returned ${tokenResponse.status}`)
          }
        } catch (error) {
